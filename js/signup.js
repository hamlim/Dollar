      ...
      $.ajax(getSettings).done(function(response) {
        ...
        for (var i = 0; i < response.records.length; i++) {
          if (username === response.records[i].fields.u_username) {
            ...
            if (
              email === response.records[i].fields.u_email &&
              password === response.records[i].fields.u_password
            ) {
              ...
              notie.alert(
                2,
                'Warning, that username is already taken, what about super' + username + '?',
                7,
              )
            }
          }
        ...
        if (!isInCurrentUsers) {
          ...
          $.ajax(postSettings).done(function(response) {
            ...
            if (response.error) {
              ...
              notie.alert(
                3,
                "We're sorry but it seems that your account wasn't made :(. If you have seen this more than once, please <a href='mailto:hamlim@outlook.com'>Email</a> us!",
                5,
              )
            } else {
