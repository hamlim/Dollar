(_ => {
  if(localStorage.length === 0){
      ...
      window.fetch('https://api.airtable.com/v0/applYClUOdBXhRzGf/Dollars?sortField=d_time&sortDirection=desc', fetchHeaders)
        .then(response => {
        ...
        .then(data => {
          ...
          for(var i=0; i<userdata.length; i++){
            if(userdata[i].fields.d_userIDFK === userID){
                //add it to the user expenses array
                userExpenses.push(userdata[i]);
            }
          }
          ...
          document.querySelector('.expense-info').addEventListener('click', _ => {
            ...
            for(let j=0; j<userExpenses.length; j++){
              if(userExpenses[j].id === uniqueRowID){
                let converted = moment(userExpenses[j].fields.d_time);
                let readable = converted._d;
                vex.dialog.alert({
        ...
        .catch(err => {
          console.warn('Failed fetch of userdata with Error: ', err);
        });
